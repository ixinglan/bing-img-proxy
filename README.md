...
**③ 回源缩图 —— 默认追加 `w/h/c/rs` 参数，且尺寸要按「设备像素」定**

Bing 的 `th` 接口支持按需缩放，不传参数时可能是 UHD 大图。实测（2026-09-16，同一张 UHD 原图）：

| 参数 | 体积 | 耗时 | 实际返回像素 |
| --- | --- | --- | --- |
| （不传参数） | 3,679,107 字节 | 16.6s | 3840×2160 |
...
**为什么默认是 3840×2160 而不是 1920×1080**：背景以 `background-size: cover` 铺满整个视口，
...
>
> **改参数后缓存会自动失效**：缓存文件名是 `sha1(image_id + "|" + 取图参数)`，
> 所以改了 `BING_IMAGE_PARAMS` 不会命中旧文件；旧文件变成孤儿，会被 LRU 按 mtime 优先淘汰。
> 想立即释放空间可以直接清空 `CACHE_DIR`。
>
> 若访客以手机为主，可把 `BING_IMAGE_PARAMS` 调小（如 `w=1920&h=1080&c=7&rs=1`）以节省流量，
> 手机视口远小于桌面。
...
| `BING_IMAGE_PARAMS` | `w=3840&h=2160&c=7&rs=1` | 回源缩图参数；置空则取原图。尺寸要按设备像素定，详见上文 ③ |
